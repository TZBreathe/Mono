...
N=1;
currData=lincc_45{N}(4:1100,1);
socData=lincc_45{N}(4:1100,3);
voltageData=lincc_45{N}(4:1100,2);
tempData=lincc_45{N}(4:1100,4);
...
LUT_lincc_45(:,1)=xOptTmp;
temp=xOptTmp;
% save param_lincc15.mat xOptTmp
%% N=2, 26 A
N=2;
currData=lincc_45{N}(4:1000,1);
socData=lincc_45{N}(4:1000,3);
voltageData=lincc_45{N}(4:1000,2);
tempData=lincc_45{N}(4:1000,4);
timeData=4:length(currData)+3;
dt=1;

k0=xOptTmp;
lowBound =[k0(1)*0.5;  k0(2)*0.5;    k0(3)*0.7];
upBound = [k0(1)*1.5;   k0(2)*2;    k0(3)*1.4];
globObj = @(k)ECN_Diffusion_Param_Optim_Function(k,currData,timeData,socData,voltageData,tempData,BrOcv,BrEcn);
[xOptTmp,fOpt(1,1),~,~] = ga(globObj,length(k0),[],[],[],[],lowBound,upBound,[],genAlgOpts);


LUT_lincc_45(:,2)=xOptTmp;
...
k0=temp;
lowBound =[k0(1)*0.5;  k0(2)*0.5;    k0(3)*0.7];
upBound = [k0(1)*2;   k0(2)*2;    k0(3)*1.4];
globObj = @(k)ECN_Diffusion_Param_Optim_Function(k,currData,timeData,socData,voltageData,tempData,BrOcv,BrEcn);
[xOptTmp,fOpt(1,1),~,~] = ga(globObj,length(k0),[],[],[],[],lowBound,upBound,[],genAlgOpts);
...
%% save lut
 save LUT_lincc_45.mat LUT_lincc_45;

 %% Test run

N=3;
% params=xOptTmp;
load('LUT_lincc_45.mat');
params=LUT_lincc;

currData_t=lincc_45{N}(4:1000,1);
socData_t=lincc_45{N}(4:1000,3);
voltageData_t=lincc_45{N}(4:1000,2);
tempData_t=lincc_45{N}(4:1000,4);
