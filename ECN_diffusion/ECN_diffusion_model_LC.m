...
function [voltOut,U_hyst]=ECN_diffusion_model_LC(k,currData,timeData,socData,tempData,OcvLuts,ECNParams)
%params

dt=timeData(2)-timeData(1);
N=1/dt; % controls timestep
...
SoCs = zeros(length(timeData),1); % concentration at surface
SoCavg=SoC0;
SoCs(1) = SoC0;
...
%     tau=tau_0;
    ...
    R0=interpn(ECNParams.Dims.soc, ECNParams.Dims.temp, ECNParams.Dims.crate,ECNParams.Components.R_0,socData(timestep),tempData(timestep),curr_Data(timestep)./4.75,'linear',ECNParams.Components.R_0(ceil(socData(timestep)*21),5,1));
    R1=interpn(ECNParams.Dims.soc, ECNParams.Dims.temp, ECNParams.Dims.crate,ECNParams.Components.R_1,socData(timestep),tempData(timestep),curr_Data(timestep)./4.75,'linear',ECNParams.Components.R_1(ceil(socData(timestep)*21),5,1));
    R2=interpn(ECNParams.Dims.soc, ECNParams.Dims.temp, ECNParams.Dims.crate,ECNParams.Components.R_2,socData(timestep),tempData(timestep),curr_Data(timestep)./4.75,'linear',ECNParams.Components.R_2(ceil(socData(timestep)*21),5,1));
    C1=interpn(ECNParams.Dims.soc, ECNParams.Dims.temp, ECNParams.Dims.crate,ECNParams.Components.C_1,socData(timestep),tempData(timestep),curr_Data(timestep)./4.75,'linear',ECNParams.Components.C_1(ceil(socData(timestep)*21),5,1));
    C2=interpn(ECNParams.Dims.soc, ECNParams.Dims.temp, ECNParams.Dims.crate,ECNParams.Components.C_2,socData(timestep),tempData(timestep),curr_Data(timestep)./4.75,'linear',ECNParams.Components.C_2(ceil(socData(timestep)*21),5,1));
...
%     
    ...
    U_hyst(timestep)=M_hyst.*h+sign(curr_Data(timestep)).*M0;
    ...
    SoC(end) = SoC(end) + (curr_Data(timestep)/3/Q)*Sa(end)*dt/dV(end); % at boundary
     SoCs(timestep) = max(0,SoC(end)); % surface soc
    SoCavg= socData(timestep);
%     SoCs(timestep) = SoC0+trapz(currData(1:timestep))*dt/(4.75*3600);
    OCVcell=interpn(OcvLuts.Dims.soc,OcvLuts.Dims.temp,OcvLuts.Components.ocv,SoCavg,tempData(timestep),'linear',OcvLuts.Components.ocv(ceil(SoCavg*1000),5));% ocv at avearge soc
    %OCVcell=interpn(OcvLuts.Dims.soc,OcvLuts.Dims.temp,OcvLuts.Components.ocv,SoCavg(timestep),tempData(timestep),'makima');
    OCVcell_surf=interpn(OcvLuts.Dims.soc,OcvLuts.Dims.temp,OcvLuts.Components.ocv,SoCs(timestep),tempData(timestep),'linear',OcvLuts.Components.ocv(max(ceil(SoCs(timestep)*1000),5),1)); %ocv at surface soc
    Vdiff=-kd*(OCVcell-OCVcell_surf);
    v_Out(timestep)=IR0+Vrc1+Vrc2+Vdiff+OCVcell+U_hyst(timestep);
